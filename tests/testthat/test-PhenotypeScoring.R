...
test_that("executePhenotypeScoring works", {
  ...
  covariatesPerPerson <- analysisResults |>
    dplyr::tbl("covariatesPerPerson") |>
    dplyr::collect()
  gwasResults <- analysisResults |>
...
# With covariates
#
test_that("executePhenotypeScoring works with includeDaysToFirstEvent = TRUE", {
  skip_if_not(Sys.getenv("HADESEXTAS_TESTING_ENVIRONMENT") == "Synthea-S10")
...
test_that(".extractCovariatesPerPerson works with includeDaysToFirstEvent = TRUE", {
  skip_if_not(Sys.getenv("HADESEXTAS_TESTING_ENVIRONMENT") == "Synthea-S10")
  # set up
  cohortTableHandler <-
    helper_createNewCohortTableHandler(addCohorts = "DiabetesSyntheaCohorts")
  ...
  covariatesTable <- dplyr::bind_rows(
    covariatesTable,
    covariatesTable |>
      dplyr::mutate(
...
test_that("executePhenotypeScoring works", {
  ...
  covariatesPerPerson <- analysisResults |>
    dplyr::tbl("covariatesPerPerson") |>
    dplyr::collect()
  codewasResults <- analysisResults |>
    dplyr::tbl("codewasResults") |>
    dplyr::collect()
  ...
  # Demographics
  nCases <- cohortTableHandler$cohortDemograpics |>
  ...
  covariatesPerPerson |>
    ...
    expect_equal(nCases)
  covariatesPerPerson |>
    dplyr::filter(covariateId == 1041) |>
    nrow() |>
    expect_equal(nCases)
  covariatesPerPerson |>
    dplyr::filter(covariateId == 1010) |>
    nrow() |>
    expect_equal(nCases)

  nMale <- cohortTableHandler$cohortDemograpics |>
    dplyr::filter(cohortId == 1) |>
    purrr::pluck("sexCounts", 1) |>
    dplyr::filter(sex == "MALE") |>
    dplyr::pull(n)
  nFemale <- cohortTableHandler$cohortDemograpics |>
    dplyr::filter(cohortId == 1) |>
    purrr::pluck("sexCounts", 1) |>
    dplyr::filter(sex == "FEMALE") |>
    dplyr::pull(n)
  covariatesPerPerson |>
    dplyr::filter(covariateId == 8507001) |>
    nrow() |>
    expect_equal(nMale)
  covariatesPerPerson |>
    dplyr::filter(covariateId == 8532001) |>
    nrow() |>
    expect_equal(nFemale)
})