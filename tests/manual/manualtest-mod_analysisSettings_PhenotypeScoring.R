...
app <- shiny::shinyApp(
  shiny::fluidPage(
    mod_analysisSettings_phenotypeScoring_ui("test")
  ),
  function(input, output, session) {
    ...
    rf_analysisSettings <- mod_analysisSettings_phenotypeScoring_server("test", r_connectionHandler)
    ...
    shiny::observe({
      ...
      if (!is.null(analysisSettings)) {
        analysisSettings |>
          assertAnalysisSettings_PhenotypeScoring() |>
          expect_no_error()
      }
    })
  },
